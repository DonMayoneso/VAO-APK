...
/**
 * PROTOCOLO HIDRA - CAPÍTULO 4
 * El Declive de la Eficiencia.
 */
...
const storyData = {
    ...
    4: {
        ...
        content: `
            <p>Pasaron cinco años. La euforia se evaporó. Mantener el ganado requería recursos que ya no existían. Los animales comenzaron a llegar a los extractores demacrados.</p><br>
            <p><strong>Alerta:</strong> Rendimiento hídrico por unidad bovina: -22% respecto al estándar.</p><br>
            <p>Las granjas comenzaron a cerrar. El miedo volvió a las calles. Los disturbios eran sofocados brutalmente por fuerzas de seguridad con trajes refrigerados.</p>
        `
    }
};
...
const app = {
    ...
    init: function() {
        ...
        console.log("Capítulo 4: Inicializado.");
    },
    ...
    tick: function() {
        if(this.state.isGameOver || !this.state.storyViewed_ch4) return;
    ...
    buyTier: function(tierId) {
        ...
        if(this.state.water >= tier.cost) {
            ...
            this.renderFarms();
            this.renderMapInfo();
            if (tierId === 5) this.triggerChapterTransition();
        } else {
            sfx.error();
        }
    },
    ...
    triggerChapterTransition: function() {
        ...
        sfx.mechanic();
        setTimeout(() => { window.location.href = "../ch5/capitulo5.html"; }, 3000);
    },
